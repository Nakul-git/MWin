...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  $id = $_POST['idVal'];
  $ses_sql = mysqli_query($conn,"SELECT * FROM `customers` WHERE `login_ID` = $id;");
}
?>
<!DOCTYPE html>
<html lang="en">
...
<nav class="navbar navbar-light navbar-expand-md sticky-top navbar-shrink py-3" id="mainNav">
    ...
    <a href="MWin.php"><-Return</a>
    <?php
    while($row = mysqli_fetch_row($ses_sql)){ 
    echo  "<form method ='post' action='modify.php' class='needs-validation' novalidate>
           <div class='edit'>
           <input name='idVal' value='$row[0]' type='hidden'>
          <span>Name: $row[1] <input name='name' placeholder='Enter new name' class='form-control' minlength=3 type='text' required></span><br><br>
           <span>Phone: $row[2] <input name='phone' placeholder='Enter new number' class='form-control' maxlength=10 minlength=10  type='text' pattern='\d{10}' required></span><br><br>
